...
id = parseInt(location.pathname.replace('/level/', '')) - 1

if (location.pathname.replace(/\/level\/(.*)/, '/level') == '/level') {
    try {
        url = location.toString().replace(location.toString().split(location.host)[1], '')
        ...
        $('.left').on('click', function () {
            if (id == 0) return
            $(location).attr('href', `${url}/level/${id}`)
        })

        $('.right').on('click', function () {
            if (id == list.length - 1) return
            $(location).attr('href', `${url}/level/${id + 2}`)
        })
